...
const ChatBotPage = () => {
  ...
  return (
    <section className="flex h-screen dark:bg-gradient-to-br dark:from-red-700 dark:via-black dark:to-red-700 w-full">
      <div
        className={`text-center lg:p-4 px-6 pt-12 relative ${
          isSidebarRightOpen
            ? "lg:max-w-4xl block max-w-fit"
            : "lg:w-[896px] w-full"
        }  mx-auto`}
      >
        ...
        <div className="bg-hitam lg:pb-16 lg:pt-10 rounded-4xl flex lg:max-w-3xl p-5 mx-auto text-sm mt-10 lg:relative lg:px-12">
          <input
            ...
            className="w-full lg:px-4 p-2 lg:py-2 rounded-lg bg-abu text-black placeholder-gray-700 focus:outline-none focus:ring-2 focus:ring-slate-300 transition duration-300"
          />
          <button className="lg:absolute lg:bottom-5 lg:left-14 text-white cursor-pointer ml-3 transition duration-300 transform hover:scale-110 ">
            <GrAdd size={27} />
          </button>
        </div>
      </div>
      <aside
        className={`text-white lg:bg-chatbotsidebar flex-shrink-0 transform transition-all fixed    right-0 overflow-auto lg:relative  overflow-y-auto duration-300 ease-in-out overflow-x-hidden z-10 ${
          isSidebarRightOpen
            ? "lg:w-[213px] w-[400px] h-full p-4 backdrop-blur-2xl"
            : "w-16 h-full ml-36 p-2 bg-transparent"
        }`}
      >
        {isSidebarRightOpen ? (
          <>
            ...
            <div className="flex flex-wrap mt-20">
              <div className="mr-6 my-3">
                <input
                  type="text"
                  placeholder="cari"
                  className="p-2 rounded-2xl shadow-2xl border-2 border-none focus:outline-none focus:ring-2 focus:ring-slate-300 transition duration-300 bg-gray-500/20 lg:w-44 w-80"
                />
              </div>
          ...
          <div className=" flex flex-col ml-3">
            ...
            <MdContentPasteSearch
              size={24}
              className="text-gray-400 lg:block hidden cursor-pointer my-5 transition-transform duration-300 hover:scale-105"
              onClick={toggleSidebar}
            />
            <PiNotePencil
              size={24}
