...
const SideBar = () => {
  ...
  const getBorderColor = (path: string) => {
    return pathname === path ? "border-2 rounded-xl pl-2 pr-10 py-2 border-blue-500 text-blue-400" : "border-white";
  };

  return (
    <aside className={`min-w-64 max-w-64 hidden lg:flex flex-col gap-10 items-center min-h-svh px-5 border-r-2 border-slate-500`}>
      <section>
        <nav className={`py-5`}>
      ...
      <section className="w-full">
        <nav>
          <ul className={`flex flex-col gap-4 font-bold text-xl ${concertOne.className}`}>
